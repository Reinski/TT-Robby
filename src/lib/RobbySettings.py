...
class RobbySettings:
    ...
    def __init__(self, ballspeed = 1.0, topspin = 0.0, sidespin = 0.0, ballfreq = 0.2) -> None:
        ...
        self.BallFeederCycle = RobbySettings.DEFAULT_BALL_FEEDER_CYCLE
        self.BallFeederMountIndex = RobbySettings.DEFAULT_BALL_FEEDER_MOUNT_INDEX
    ...
    def __set_ball_feeder_cycle(self, value: List[int]) -> None:
        self.__ball_feeder_cycle = value
        # ensure that full cycles are performed (add any missing steps)
        if sum(value) != 0:
            self.__ball_feeder_cycle.append(int(-sum(value)))

    def __set_ball_feeder_mount_index(self, value: int) -> None:
        if value > len(self.__ball_feeder_cycle) or value < 0:
            value = len(self.__ball_feeder_cycle) - 1
        self.__ball_feeder_mount_index = value
    ...
    BallFeederCycle = property(lambda self: self.__ball_feeder_cycle, __set_ball_feeder_cycle)
    """Define the motor movement for the ball feeder"""
    BallFeederMountIndex = property(lambda self: self.__ball_feeder_mount_index, __set_ball_feeder_mount_index)
    """Define the motor position when mounting the ball feeder"""
    ...
    def getConfigData(self) -> dict:
        return {
            ...
            'default_ball_feeder_cycle': self.DEFAULT_BALL_FEEDER_CYCLE,
            'default_ball_feeder_mount_index': self.DEFAULT_BALL_FEEDER_MOUNT_INDEX,
    ...
    def load_from_config(self, config: dict):
        ...
        if 'default_ball_feeder_cycle' in config:
            self.DEFAULT_BALL_FEEDER_CYCLE = config['default_ball_feeder_cycle']
        if 'default_ball_feeder_mount_index' in config:
            self.DEFAULT_BALL_FEEDER_MOUNT_INDEX = int(config['default_ball_feeder_mount_index'])
